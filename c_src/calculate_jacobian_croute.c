...
void mysyrk(double const * A, double * B, int n, int rank, double* Bindices, int idx)
{
    int i, j, k;
    for(i = 0; i< n;i++){
        for(j = 0; j<n;j++){        
            Bindices[(j*n+i)*2]=idx*n+i+1;
            Bindices[(j*n+i)*2+1]=idx*n+j+1;            
	        for(k = 0;k<rank;k++)
...
void mygemmNT(double alpha, double const *A, double const *B,double * C, int n, int rank, int m, double* Eindices, int camera, int point)
  {
    int i, j, k;
    for (i = 0;i <n; i++){
        for(j = 0; j<m;j++){
            Eindices[(j*n+i)*2]=camera*9+j+1;
            Eindices[(j*n+i)*2+1]=point*3+i+1;
	        for (k =0;k<rank;k++)
...
void calculate_jacobian(double * state_x, 
			       ...
			       double* obs_mask_precon, mwSize obs_mask_precon_length, double* obs_mask, mwSize obs_mask_length, double* f, double* f2,
			       double * B, double * C, double * E, double * g,
			       double* Bindices, double* Cindices, double* Eindices, double* B2, double* C2, 		               double* E2, double* g2, double* B2indices, double* C2indices, double* E2indices,
			       int n_inputs
			       )
{
  if(n_inputs==8) 
  { 
    ...
    for(iter = 0; iter< n_obs; ++iter){
        ...
        mysyrk(buffer,B+camera*81, 9, 2, Bindices+camera*81*2, camera);
        mygemv(buffer,f+iter*2,g+camera*9, 9, 2);
        mygemmNT(1,buffer+18,buffer,E+iter*27, 3, 2, 9, Eindices+iter*27*2, camera, point);
        mysyrk(buffer+18,C+point*9, 3, 2, Cindices+point*9*2, point);
  ...
  else if(n_inputs==11)
  {
    ...
    for(iter = 0; iter< n_obs; ++iter){
        ...
        {                 
            mysyrk(buffer,B2+camera*81, 9, 2, B2indices+camera*81*2, camera);
            mygemv(buffer,f2+iter*2,g2+camera*9, 9, 2);
            mygemmNT(1,buffer+18,buffer,E2+iter*27, 3, 2, 9, E2indices+iter*27*2, camera, point);
            mysyrk(buffer+18,C2+point*9, 3, 2, C2indices+point*9*2, point);
            mygemv(buffer+18,f2+iter*2,g2+n_cams*9+point*3, 3, 2);
...
{    
    if(nlhs!=14) {
        mexErrMsgIdAndTxt("MyToolbox:arrayProduct:nlhs","14 outputs required.");
    }    
    double* state_x=mxGetPr(prhs[0]);    
    mwSize n_cams=mxGetScalar(prhs[1]);
    mwSize n_obs=mxGetScalar(prhs[2]);
    mwSize n_pts=mxGetScalar(prhs[3]);
    double* cidx=mxGetPr(prhs[4]);
    double* pidx=mxGetPr(prhs[5]);
    double* D=mxGetPr(prhs[6]);
    ...
    if(nrhs==8){
        ...
        obs_mask_precon=mxGetPr(prhs[7]);
        obs_mask_precon_length=mxGetScalar(prhs[8]);
        f=mxGetPr(prhs[9]);
        f2=mxGetPr(prhs[10]);        
    }else if(nrhs==13){
        obs_mask_precon=mxGetPr(prhs[7]);
        obs_mask_precon_length=mxGetScalar(prhs[8]);
        obs_mask=mxGetPr(prhs[9]);
        obs_mask_length=mxGetScalar(prhs[10]);
        f=mxGetPr(prhs[11]);
        f2=mxGetPr(prhs[12]);   
    }else{
        mexErrMsgIdAndTxt("MyToolbox:arrayProduct:nrhs","8, 11 or 13 inputs required.");
    }
    ...
    plhs[6]=mxCreateDoubleMatrix(3*n_pts+9*n_cams, 1, mxREAL); 
    double* g=mxGetPr(plhs[6]);
    plhs[8]=mxCreateDoubleMatrix(81*n_cams*2, 1, mxREAL);
    double* Bindices=mxGetPr(plhs[8]);
    plhs[9]=mxCreateDoubleMatrix(9*n_pts*2, 1, mxREAL);
    double* Cindices=mxGetPr(plhs[9]);
    plhs[10]=mxCreateDoubleMatrix(27*n_obs*2, 1, mxREAL);
    double* Eindices=mxGetPr(plhs[10]);
    ...
    double* E2indices;
    if(nrhs!=8){
        plhs[3]=mxCreateDoubleMatrix(81*n_cams, 1, mxREAL); 
        B2=mxGetPr(plhs[3]);
        plhs[4]=mxCreateDoubleMatrix(9*n_pts, 1, mxREAL); 
        C2=mxGetPr(plhs[4]);
        plhs[5]=mxCreateDoubleMatrix(27*n_obs, 1, mxREAL); 
        E2=mxGetPr(plhs[5]);
        plhs[7]=mxCreateDoubleMatrix(3*n_pts+9*n_cams, 1, mxREAL); 
        g2=mxGetPr(plhs[7]);
        plhs[11]=mxCreateDoubleMatrix(81*n_cams*2, 1, mxREAL);
        B2indices=mxGetPr(plhs[11]);
        plhs[12]=mxCreateDoubleMatrix(9*n_pts*2, 1, mxREAL);
        C2indices=mxGetPr(plhs[12]);
        plhs[13]=mxCreateDoubleMatrix(27*n_obs*2, 1, mxREAL);
        E2indices=mxGetPr(plhs[13]);        
    }else{
    ...
    calculate_jacobian(state_x, n_cams, n_obs, n_pts, cidx, pidx, D, obs_mask_precon, obs_mask_precon_length, obs_mask, obs_mask_length, f, f2, B, C, E, g, Bindices, Cindices, Eindices, B2, C2, E2, g2, B2indices, C2indices, E2indices, nrhs);   
