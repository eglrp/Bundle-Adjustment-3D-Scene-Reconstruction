...
void mysyrk(double const * A, double * B, int n, int rank, int idx)
{
    int i, j, k;
    for(i = 0; i< n;i++){
        for(j = 0; j<n;j++){
...
void mygemmNT(double alpha, double const *A, double const *B,double * C, int n, int rank, int m, int camera, int point)
  {
    int i, j, k;
    for (i = 0;i <n; i++){
        for(j = 0; j<m;j++){           
...
void calculate_jacobian(double * state_x, 
			       ...
			       double mu, double * B, double * C, double * E, double * g, double* diag_weights)
{  
  double buffer[24];
  int iter;
  for(iter = 0; iter< n_obs; ++iter){
    ...
    mysyrk(buffer,B+camera*81, 9, 2, camera);
    mygemv(buffer,f+iter*2,g+camera*9, 9, 2);
    mygemmNT(1,buffer+18,buffer,E+iter*27, 3, 2, 9, camera, point);
    mysyrk(buffer+18,C+point*9, 3, 2, point);
    mygemv(buffer+18,f+iter*2,g+n_cams*9+point*3, 3, 2);
  }
  
  double min_diag=1e-6;
  double max_diag=1e+32;
  double* ptr=B;
  int i, j;
  for(i = 0; i < n_cams; ++i){
      for(j = 0; j < 9; ++j){
            
	        double d = mu*min(max(ptr[j*10],min_diag),max_diag);   		   
	        if(i==0) 
	            diag_weights[j] = d;
  ...
  for(i = 0; i < n_pts; ++i){
     for(j = 0; j < 3; ++j){
       double d = mu*min(max(ptr[j*4],min_diag),max_diag);
       ptr[j*4] += d; 
     };
     ptr += 9;
   };
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    if(nlhs!=5) {
        mexErrMsgIdAndTxt("MyToolbox:arrayProduct:nlhs","5 outputs required.");
    }    
    ...
    double* diag_weights=mxGetPr(plhs[4]);   
    
    calculate_jacobian(state_x, f, n_cams, n_obs, n_pts, cidx, pidx, D, mu, B, C, E, g, diag_weights);     
};
