...
%
% Parameters:   A           3xN or 3Nx1 matrix representing the N 3D points
%               B           3xN or 3Nx1 matrix representing the N 3D points
...
function [s R T Bi err] = absoluteOrientationQuaternion( A, B, doScale)
...
if(c1~=3 || c2~=3)
    A=[A(1:3:size(A, 1)-2, 1)'; A(2:3:size(A, 1)-1, 1)'; A(3:3:size(A, 1), 1)'];
    B=[B(1:3:size(B, 1)-2, 1)'; B(2:3:size(B, 1), 1)'; B(3:3:size(B, 1), 1)'];
    [c1 r1] = size(A);
    [c2 r2] = size(B);
end
if(r1<4)
